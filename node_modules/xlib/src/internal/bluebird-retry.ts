 ...
 */
function StopError(this: any, err: string | Error) {
    this.name = 'StopError';
...
(retry as any).StopError = StopError;
...
function retry<TValue>(func: () => Promise<TValue>, options?: any): Promise<TValue> {
    options = options || {};

    var interval: number = typeof options.interval === 'number' ? options.interval : 1000;

    var max_tries: number|null = null;
    var giveup_time: number | null = null;
    ...
    // that should retry.
    var predicate = options.predicate || function (err:any) { return true; }
    var stopped = false;

    function try_once(): any {
        ...
        return Promise.attempt(function () {
            ...
            .caught(predicate, function (err) {
                ...
                if ((max_tries && (tries === max_tries) ||
                    ...
                    var timeout: any = new Error('operation timed out after ' + (now - start) + ' ms, ' + tries + ' tries with error: ' + err.message);
                    timeout.failure = err;
...
function backoff(interval: number, options: any) {
    if (options.backoff) {
